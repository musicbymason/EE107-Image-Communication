...
\title{EE107 Final Report}
\author{Jacob Gerson, Asher Milberg, Mason Doshi}
\date{April 2026}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{imgs/macjones.jpg}
    \caption{Test image}
\end{figure}

\newpage

\paragraph{Q1: Pulse with more bandwidth}

Here are the time-domain plots

\textbf{Half Sine Wave:}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q1/Q1_tsrrc.jpg}
\end{figure}

\textbf{SRRC Wave:} 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q1/Q1_tk3.jpg}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q1/Q1_fsrrc.jpg}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{"imgs/Q1/Q1_fa0,1.jpg"}
\end{figure}

The half-sine frequency response uses more bandwidth that the SRRC pulse. The SRRC response has a flat response until $\sim 0.5$ Hz (which is shown by our rolloff factor alpha), and then drops off. 

In contrast, the half sine response decays very slowly - even at 5 Hz there is only $\sim 30$ dB drop off whereas at 5 Hz our SRRC response has dropped off by more than 100 dB.
...
Increasing K does not change the bandwidth of the SRRC pulse, since bandwidth is determined by $\alpha$ and $T$. However, a larger K reduces truncation error by preserving more of the pulse's decaying tails. This results in a frequency response that is closer to the ideal SRRC spectrum. A small K truncates the tails more, causing energy leakage and ripples visible in the frequency domain.

\textbf{SRRC time and frequency domain plots with k = 3:}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q1/Q1_fk3.jpg}
\end{figure}

\textbf{SRRC time and frequency domain plots with k = 6, alpha = 0.1:}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{"imgs/Q1/Q1_ta0,1.jpg"}
\end{figure}
...
\subparagraph{Half-Sine Pulse Characteristics}
...
\subparagraph{Square Root Raised Cosine (SRRC) Characteristics}
...
\paragraph{Q4: Modulated Signal Eye Diagrams}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q4/Q4_hseye.jpg}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q4/Q4_srrceye.jpg}
\end{figure}
...
\paragraph{Q5: Frequency and Impulse Response of the Channel}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q5/Q5_Channel_Responses.jpg}
\end{figure}

\paragraph{Q6: Eye Diagram of Channel Output}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q6/After_Channel_Eye_HS.jpg}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q6/After_Channel_Eye_SRRC.jpg}
\end{figure}
...
\paragraph{Q7: Noisy Eye Diagram of Channel Output}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q7/Combined_Noise_Analysis.jpg}
\end{figure}
...
\paragraph{Q8: Matched Filter Output - Impulse and Frequency Response Graphs}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q8/matched.jpg}
\end{figure}
...
\paragraph{Q9: Matched Filter Output - Eye Diagrams for 1 \& 2 Bit Durations}

\textbf{Matched Filter Eye Diagram - 1 Bit Duration}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q9/Matched_Filter_Eyes_1bit.jpg}
\end{figure}

\textbf{Matched Filter Eye Diagram - 2 Bit Duration}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/Q9/Matched_Filter_Eyes.jpg}
\end{figure}
...
\paragraph{Q10: The Zero-Forcing (ZF) Equalizer}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q10/Q10.jpg}
\end{figure}
...
\paragraph{Q11: Zero-Forcing (ZF) Equalizer Eye Diagrams}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q11/ZF_Eyes_Combined.jpg}
\end{figure}
...
\textbf{Zero-Forcing Time Domain Output (10-bit stream):}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q11/ZF_time_10bit.jpg}
\end{figure}

These results are an updated graph to show the progess of our information along the communication system's path. 

\paragraph{Q12 The MMSE Equalizer}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q12/MMSE_freq.jpg}
\end{figure}
...
\paragraph{Q13: The MMSE Equalizer Eye Diagrams}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q13/MMSE_eye.jpg}
\end{figure}

At $\sigma^2 = 0$, both pulse shapes have an open eye, with the half-sine appearing more tightly clustered while the SRRC is more spread out due to its longer pulse duration and overlap. Even in the noiseless case, the SRRC traces are less compact, which means there's more residual ISI, but both still have a visible sampling point. As noise increases to $\sigma^2 = 0.005$ and $0.020$, the eye openings begin to close for both pulses, but the traces remain bounded, indicating that the MMSE equalizer is limiting noise amplification.

\textbf{MMSE Time Domain Output (10-bit stream):}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q13/MMSE_time_10bit.jpg}
\end{figure}

This behavior is consistent in the aligned time-domain outputs. At $\sigma^2 = 0$, both pulse shapes track the transmitted symbols, with separation between $\pm 1$ levels. As noise increases, the signals become more distorted, but are still stable and follow the overall symbol transitions. Even at $\sigma^2 = 0.020$, although the signal is noisy and it's still hard to determine what's a 0 and a 1, the waveform structure is still there, showing that the MMSE equalizer trades off perfect channel inversion for improved robustness to noise.

\newpage

\paragraph{Q14: Final Image Transmission Comparison}

Below is the \textbf{grayscale reference image} that is processed and transmitted through the system (after 8x8 DCT block processing and bit conversion):

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{imgs/Q14/Reference_Gray.jpg}
    \caption{Grayscale Reference}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imgs/Q14/Final_Result.jpg}
\end{figure}
...
\paragraph{Q15: Critical SNR Thresholds}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q15/Q15_BER_Curves.jpg}
\end{figure}

\paragraph{Q16: Performance on Different Images}

Below are the results for doing the same analysis and modulation on a picture of Raven's QB Lamar Jackson:

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imgs/Q16/Final_Result_lamar.jpg}
\end{figure}
...
\paragraph{Q17: Nyquist Criterion and Zero ISI}

Again, from Q9, here is the eye diagram of both pulses across noise environments: 

\textbf{Matched Filter Output Eye Diagram}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/Q9/Matched_Filter_Eyes.jpg}
\end{figure}
...
\paragraph{Q18: Error Performance at Equal Energy}
...
We used the images that were produced for Q14. At a $\sigma^2$ of 0.005 (with all other variables kept constant) and looking at the MMSE equlizer results, we can see that the SRRC pulse does much better. The images are very similar but there is just slightly less noise in the SRRC output. Even though both pulses have the same energy, the SRRC is more robust to noise.

\paragraph{Q19: Bandwidth and Pulse Length}
...
\paragraph{Q20: Conclusion on Pulse Shaping}
...
\paragraph{Q21: Performance Under New Channels}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{imgs/Q21/Recovered_Outdoor.jpg}
\end{figure}

\subsubsection*{Indoor Channel}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{imgs/Q21/Recovered_Indoor.jpg}
\end{figure}
